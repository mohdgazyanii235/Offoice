  ...
  public static String mainConvert(double number) {
    ...
    for (int i = 0; i < stringNumber.length(); i++) {
      if (stringNumber.charAt(i) == '.') {
        ...
        decimalNumber += stringNumber.charAt(i);
      } else {
        wholeNumber += stringNumber.charAt(i);
      }
    }

    double doubleWholeNumber = Double.parseDouble(wholeNumber);
    double doubleDecimalNumber = Double.parseDouble(decimalNumber);

    return SpellNumber.convert(doubleWholeNumber) + " Dollars and "
        + SpellNumber.convert(doubleDecimalNumber) + " Cents";
